...
- Stack is fixed: React + Vite + Tailwind + shadcn/ui on frontend, Node + Express on backend, OpenAI API for all AI features. Do not suggest or switch to a different stack or AI provider.
...
- Categorisation must be hybrid: a deterministic rule engine first, OpenAI only for unmatched transactions. Never make categorisation pure-LLM.
- All numeric insights (totals, savings rate, cash flow, aggregates) are computed in code, never by an LLM.
- Account numbers are masked to last 4 digits everywhere — in the UI and before sending anything to OpenAI.
